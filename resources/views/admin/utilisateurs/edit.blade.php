...
<div class="max-w-3xl mx-auto">
    ...
    <div class="bg-white rounded shadow p-6 mt-4">
        <h2 class="text-2xl font-bold">Modifier {{ $user->name }}</h2>
        ...
        <form action="{{ route('admin.utilisateurs.update', $user->id) }}" method="POST" class="mt-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $user->name) }}" class="mt-1 w-full rounded border px-3 py-2">
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700">Email</label>
                    <input type="email" name="email" value="{{ old('email', $user->email) }}" class="mt-1 w-full rounded border px-3 py-2">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                ...
                <div>
                    ...
                    <input type="number" name="token" value="{{ old('token', $user->employe->token ?? 0) }}" class="mt-1 w-full rounded border px-3 py-2">
                </div>
            </div>
