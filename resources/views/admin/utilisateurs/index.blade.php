                    ...
                    @foreach($utilisateurs as $user)
                        <tr class="hover:bg-slate-50 transition">
                            ...
                            <td class="px-6 py-4 text-center flex justify-center gap-2">
                                <a href="" class="px-3 py-1 bg-slate-100 text-slate-700 rounded hover:bg-slate-200">Voir</a>
                                @can('update', $user)
                                    <a href="{{ route('admin.utilisateurs.edit', $user->id) }}"
                                        class="px-3 py-1 bg-blue-100 text-blue-700 rounded hover:bg-blue-200">Modifier</a>
                                @endcan
                                ...
                                @can('delete', $user)
                                    <form action="{{ route('admin.utilisateurs.destroy', $user->id) }}" method="POST"
                                        ...
                                        @method('DELETE')
                                        <button type="submit"
                                            class="px-3 py-1 bg-rose-100 text-rose-700 rounded hover:bg-rose-200">Supprimer</button>
                                    </form>
                                @endcan
                            </td>
                        </tr>
